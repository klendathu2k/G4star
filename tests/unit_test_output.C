...
const int longtest=10;
//______________________________________________________________________
void unit_test_output() {
  ...
  LOG_TEST << "=======================================================" << std::endl;
  LOG_TEST << Form("Unit testing of file I/O for %i pi+/- tracks",longtest)     << std::endl;
  LOG_TEST << "=======================================================" << std::endl;
  ...
  if ( !hasOutput ) {
    LOG_TEST << "Please specify an output file with --output=filename" << std::endl;
    return;
  }

  auto verifyChainOption = [=](std::string opt)->void
  {
    bool hasit = chain->GetOption(opt.c_str());
    //LOG_TEST << "Chain has the /"<< opt.c_str() << "/ option " << (hasit)?PASS:FAIL << std::endl;   
    if ( hasit ) LOG_TEST << "Chain has the option " << opt << std::endl;
    else         LOG_TEST << "Chain is missing the option " << opt << std::endl;
    assert(hasit);
    return;
  };

  verifyChainOption("geant4out");


  auto generateSectorScan = [&](int sector )->void {
