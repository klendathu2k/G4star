...
#define LOGGERMESSAGE
#warning AgML compiled w/out misalignment support in ROOT6
#endif       
#include <algorithm>
#include <iostream>
  ...
  if ( ttable )
    {
      ...
      if (1 <= debuglvl) LOG_INFO << "DB Matrix: " << tablename.Data() << " [" << row << "]" << ENDL;
	...
	LOG_INFO << Form( "[%9.5f %9.5f %9.5f | %9.5f]", matrix[0][0], matrix[0][1], matrix[0][2], matrix[0][3] ) << ENDL;
	LOG_INFO << Form( "[%9.5f %9.5f %9.5f | %9.5f]", matrix[1][0], matrix[1][1], matrix[1][2], matrix[1][3] ) << ENDL;
	LOG_INFO << Form( "[%9.5f %9.5f %9.5f | %9.5f]", matrix[2][0], matrix[2][1], matrix[2][2], matrix[2][3] ) << ENDL;
	LOG_INFO << Form( "[%9.5f %9.5f %9.5f | %9.5f]", matrix[3][0], matrix[3][1], matrix[3][2], matrix[3][3] ) << ENDL;
    ...
    {
      LOG_WARN << "DB Table not found: " << table << " [" << row << "]" << ENDL;
      matrix[0][0] = 1;
      matrix[0][1] = 0;
      matrix[0][2] = 0;
      matrix[0][3] = 0;
...
bool AgPosition::Misalign( const char* tablename, const int rownumber, const char* options )
{
  ...
  if ( verbose ) { 
    LOG_INFO << "Matrix before misalignment" << ENDL;
    LOG_INFO << Form( "%8.5f   %8.5f   %8.5f | %8.5f", mMatrix[0][0],mMatrix[0][1],mMatrix[0][2],mMatrix[0][3]) << ENDL;
    LOG_INFO << Form( "%8.5f   %8.5f   %8.5f | %8.5f", mMatrix[1][0],mMatrix[1][1],mMatrix[1][2],mMatrix[1][3]) << ENDL;
    LOG_INFO << Form( "%8.5f   %8.5f   %8.5f | %8.5f", mMatrix[2][0],mMatrix[2][1],mMatrix[2][2],mMatrix[2][3]) << ENDL;
    LOG_INFO << Form( "%8.5f   %8.5f   %8.5f | %8.5f", mMatrix[3][0],mMatrix[3][1],mMatrix[3][2],mMatrix[3][3]) << ENDL;
    std::cout << std::endl;

    LOG_INFO << "Misalignment matrix (" << options << ")" << ENDL;
    LOG_INFO << Form( "%8.5f   %8.5f   %8.5f | %8.5f", M[0][0],M[0][1],M[0][2],M[0][3]) << ENDL;
    LOG_INFO << Form( "%8.5f   %8.5f   %8.5f | %8.5f", M[1][0],M[1][1],M[1][2],M[1][3]) << ENDL;
    LOG_INFO << Form( "%8.5f   %8.5f   %8.5f | %8.5f", M[2][0],M[2][1],M[2][2],M[2][3]) << ENDL;
    LOG_INFO << Form( "%8.5f   %8.5f   %8.5f | %8.5f", M[3][0],M[3][1],M[3][2],M[3][3]) << ENDL;
    std::cout << std::endl;
  ...
  if ( verbose ) { 
    LOG_INFO << "Matrix after misalignment" << ENDL;
    LOG_INFO << Form( "%8.5f   %8.5f   %8.5f | %8.5f", mMatrix[0][0],mMatrix[0][1],mMatrix[0][2],mMatrix[0][3]) << ENDL;
    LOG_INFO << Form( "%8.5f   %8.5f   %8.5f | %8.5f", mMatrix[1][0],mMatrix[1][1],mMatrix[1][2],mMatrix[1][3]) << ENDL;
    LOG_INFO << Form( "%8.5f   %8.5f   %8.5f | %8.5f", mMatrix[2][0],mMatrix[2][1],mMatrix[2][2],mMatrix[2][3]) << ENDL;
    LOG_INFO << Form( "%8.5f   %8.5f   %8.5f | %8.5f", mMatrix[3][0],mMatrix[3][1],mMatrix[3][2],mMatrix[3][3]) << ENDL;
