...
public:
  ...
  string name()      const {
    return (pdePtr) ? pdePtr->name(idSave) : " ";}
  string nameWithStatus(int maxLen = 20) const;
  int    spinType()  const {
    return (pdePtr) ? pdePtr->spinType() : 0;}
  int    chargeType() const {
    return (pdePtr) ? pdePtr->chargeType(idSave) : 0;}
  double charge()    const {
    return (pdePtr) ?  pdePtr->charge(idSave) : 0;}
  bool   isCharged() const {
    return (pdePtr) ? (pdePtr->chargeType(idSave) != 0) : false;}
  bool   isNeutral() const {
    return (pdePtr) ? (pdePtr->chargeType(idSave) == 0) : false;}
  int    colType()   const {
    return (pdePtr) ? pdePtr->colType(idSave) : 0;}
  double m0()        const {
    return (pdePtr) ? pdePtr->m0() : 0.;}
  double mWidth()    const {
    return (pdePtr) ? pdePtr->mWidth() : 0.;}
  double mMin()      const {
    return (pdePtr) ? pdePtr->mMin() : 0.;}
  double mMax()      const {
    return (pdePtr) ? pdePtr->mMax() : 0.;}
  double mass()      const {
    return (pdePtr) ? pdePtr->mass() : 0.;}
  double constituentMass() const {
    return (pdePtr) ? pdePtr->constituentMass() : 0.;}
  double tau0()      const {
    return (pdePtr) ? pdePtr->tau0() : 0.;}
  bool   mayDecay()  const {
    return (pdePtr) ? pdePtr->mayDecay() : false;}
  bool   canDecay()  const {
    return (pdePtr) ? pdePtr->canDecay() : false;}
  bool   doExternalDecay() const {
    return (pdePtr) ? pdePtr->doExternalDecay() : false;}
  bool   isResonance() const {
    return (pdePtr) ? pdePtr->isResonance() : false;}
  bool   isVisible() const {
    return (pdePtr) ? pdePtr->isVisible() : false;}
  bool   isLepton()  const {
    return  (pdePtr) ? pdePtr->isLepton() : false;}
  bool   isQuark()   const {
    return  (pdePtr) ? pdePtr->isQuark() : false;}
  bool   isGluon()   const {
    return (pdePtr) ? pdePtr->isGluon() : false;}
  bool   isDiquark()   const {
    return  (pdePtr) ? pdePtr->isDiquark() : false;}
  bool   isParton()   const {
    return (pdePtr) ? pdePtr->isParton() : false;}
  bool   isHadron()  const {
    return (pdePtr) ? pdePtr->isHadron() : false;}
