  ...
  mFilter(0),mAccepted(0)
{
  ...
  LOG_INFO << "Creating instance of StarPrimaryMaker" << endm;

  assert(fgPrimary == 0); // cannot create more than one primary generator
  fgPrimary = this;

  LOG_INFO << "Creating particle stack" << endm;
  mStack = new StarParticleStack();
  //  AgStarReader::Instance().SetStack(mStack);

  LOG_INFO << "Creating particle DB and adding as data member" << endm;
  ...
  LOG_INFO << "Setting default filter options" << endm;
