   ...
   if (!fRootFile->IsWritable()) {
      ...
      if (listOfKeys) {
         ...
         if (fDirection == kIterForward) {
            fCursorPosition = 0;
            fCurCursor              = TListIter::TObjLinkPtr_t( fList->FirstLink() );
            if (fCurCursor) fCursor = TListIter::TObjLinkPtr_t( fCurCursor->Next() );
         } else {
            fCursorPosition = fList->GetSize()-1;
            fCurCursor              = TListIter::TObjLinkPtr_t( fList->LastLink()  );
            if (fCurCursor) fCursor = TListIter::TObjLinkPtr_t( fCurCursor->Prev() );
         }
      }
   }
}
         ...
         if (0 <= newPos && newPos < collectionSize) {
            do {
               if (fCursorPosition < newPos) {
                  fCursorPosition++;
                  fCurCursor = fCursor;
                  fCursor    = TListIter::TObjLinkPtr_t(fCursor->Next());
               } else if (fCursorPosition > newPos) {
                  fCursorPosition--;
                  fCurCursor = fCursor;
                  fCursor    = TListIter::TObjLinkPtr_t(fCursor->Prev());
               }
            ...
            if (newPos < 0) {
               fCursorPosition = -1;
               if (fList) fCursor = TListIter::TObjLinkPtr_t(fList->FirstLink());
            } else  {
               fCursorPosition = collectionSize;
               if (fList) fCursor = TListIter::TObjLinkPtr_t(fList->LastLink());
            }
         }
      }
   }
